...
def handle_local_command(user_input):
    ...
    # Raw version keeps punctuation (dots in filenames survive here).
    # Wake prefix is still removed so "Hey Aurix read file notes.txt" works.
    raw_command = user_input.lower().strip()
    raw_command = re.sub(r"^\s*(hey\s+)?aurix[, ]*", "", raw_command)
...
""")
    ...
    # Read a local file
    if raw_command.startswith("read file "):
        file_name = raw_command.replace("read file ", "", 1).strip()
    ...
    if raw_command.startswith("summarize file "):
        file_name = raw_command.replace("summarize file ", "", 1).strip()

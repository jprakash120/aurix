...
def wake_listen_once():
    ...
    wake_words = ["hey aurix", "aurix", "hey oryx", "oryx"]
    ...
    for wake_word in wake_words:
        if wake_word in command:
            detected = True
            command_after_wake = command.replace(wake_word, "").strip()
            break
...
print("AURIX v0.7.1 is online.")
print("Text brain + memory + voice + microphone + laptop commands + one-time auto listen are active.")
print()
...
speak("Aurix version zero point seven point one is online. One time auto listening is active.")
